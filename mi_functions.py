...
# Olivine addition
def olvadd(wtp_olv, incr, eq_olv, major_olvadd, major):
...
def CO2add(VBvol,H2O,CO2,maj,columns,Tglass,Pglass,mfCO2,mvCO2):
    ...
    out, err = VolatileCalc('sp','basalt',[H2O,CO2new,SIO2new,Tgnew])
    Pnew = out[0]
...
###############################################################################
# MIMiC error messages
###############################################################################
class EM():
    SIO2outside = 'SIO2 outside VolatileCalc compositional range'
    Passumption = 'No water information, pressure assumed to be 100 MPa'
